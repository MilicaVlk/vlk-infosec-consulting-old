...
document.querySelectorAll('.nav-links a').forEach(link=>{if(new URL(link.href,window.location.origin).pathname===window.location.pathname)link.setAttribute('aria-current','page');});
...
if(enquiry){
 ...
 let current=0;const savedScopes={};const generalTopics=['general','partnership','speaking','existing'];const general=()=>generalTopics.includes(service.value);const activeSteps=()=>general()?[0,2]:[0,1,2];
 const questions={quickscan:['Assessment scope',['Not sure yet','Whole organization at a high level','One business unit or location','One service or platform']],customer:['Customer request',['Not sure yet','One security questionnaire','Tender / procurement evidence','Several customer requests']],governance:['Primary framework or regulation',['Not sure yet','ISO/IEC 27001','NIS2','DORA','Serbian Information Security Law','NIST CSF','Multiple / other requirements']],fractional:['Security leadership priority',['Not sure yet','Set direction and responsibilities','Coordinate an existing security program','Support management and technical teams','Prepare a longer-term roadmap']]};
 ...
 function answers(){return [...new FormData(enquiry).entries()].map(([k,v])=>[labels[k]||k,k==='service'?service.options[service.selectedIndex].text:String(v).trim()||'Not provided']);}
 function review(){const dl=document.querySelector('#answer-review');dl.replaceChildren();answers().filter(([k])=>!['Name','Organization','Email address','Role','Additional context'].includes(k)).forEach(([k,v])=>{const dt=document.createElement('dt'),dd=document.createElement('dd');dt.textContent=k;dd.textContent=v;dl.append(dt,dd);});}
 ...
 enquiry.addEventListener('submit',e=>{e.preventDefault();if(current<2){if(valid()){current=activeSteps()[activeSteps().indexOf(current)+1];show(true);}return;}if(!valid())return;const body=answers().map(([k,v])=>`${k}: ${v}`).join('\n');if(body.length>5000){feedback.textContent='Your enquiry is too long for the email application. Please shorten the message and try again.';return;}location.href='mailto:service-enquiries@vlk-infosec.com?subject='+encodeURIComponent('VLK enquiry — '+service.options[service.selectedIndex].text)+'&body='+encodeURIComponent(body);feedback.textContent='Your enquiry has been prepared. Complete sending in your email application; nothing has been submitted by this page.';});
 show();
}
